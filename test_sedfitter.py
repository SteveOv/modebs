...
from libs.fit_sed import SedFitter


if __name__ == "__main__":
    # Read the pre-built bt-settl model file
    model_sed = ModelSed("libs/data/pyssed/model-bt-settl-recast.dat")
    print("Loaded previously built dataset of solar metallicity pyssed spectra covering:")
    ...
    fitter = SedFitter(x=model_sed.get_filter_indices(sed["sed_filter"]),
                        y=sed["sed_der_flux"].quantity.to(u.Jy).value,
                        y_err=sed["sed_eflux"].quantity.to(u.Jy).value,
                        flux_func=model_sed.get_fluxes,
                        teffs=None,
                        radii=None,
                        loggs=[target_data["logg_sys"].n] * 2,
                        dist=target_data["skycoords"].distance.to(u.m).value,
                        teff_ratios=[target_data["teff_ratio"].n] * 1,
                        radius_ratios=[target_data["k"].n] * 1,
                        logg_ratios=None,
                        nstars=2,
                        verbose=True)


    soln = fitter.minimize_fit(teffs=target_data["teffs0"], radii=[1.0, 1.0])
    theta_fit = soln.x

    fit_teffs, fit_rads = theta_fit[:2].tolist(), theta_fit[2:].tolist()
    sampler = fitter.mcmc_fit(teffs=fit_teffs, radii=fit_rads, niters=20000, processes=2)
    ...
    theta_labels = [("TeffA", model_sed.teff_range.unit), ("TeffB", model_sed.teff_range.unit), ("RA", u.Rsun), ("RB", u.Rsun)]
