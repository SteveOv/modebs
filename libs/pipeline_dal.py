""" Data access components for reading/writing pipeline working data """
...
class DalDataRow(_AbstractContextManager):
    ...
    def __init__(self,
        ...
        """
        Generic representation of a row of data from the underlying data store with support for
        get/set of values and mass value getter & setter. This is a ContextManager with data
    ...
    def __setattr__(self, col: str, value):
        """
        Handles the default set behaviour for attributes. Sets the value of the correspondingly
        named column in the row's data and the corresponding uncertainty if a _err column exists.
        """
        if self._read_only:
    ...
    def _read_col_value(cls, values: _ArrayLike, col: str):
        """ Low level read from data row with additional parsing for potentially masked values. """
        value = values[col]
    ...
    def _set_col_value(cls, values: _ArrayLike, col: str, value: any):
        """ Low level write of the requested value to the underlying row while handling units. """
        values[col] = cls._append_column_unit(values[col], value)

    @classmethod
    def _append_column_unit(cls, column: any, value: any):
        """ Applies the indicated column's unit, if given by the data row, to the passed value. """
...
class Dal(_ABC):
    """
    Base **Data Access Layer** for reading/writing simple table data. This supports adding and
...
class QTableDal(Dal):
    """ Pipeline Dal for reading/writing of an in memory, masked astropy QTable. """
    ...
    def __init__(self):
        """
        Initializes the QTableDal Dal class which uses an in memory, masked astropy QTable for
        storage. This Dal is not durable and should not be used where permanent storage is required.
        """
        super().__init__()
        self._table = _QTable(masked=True, # Support for None/null as masked values
...
class QTableFileDal(QTableDal):
    """ Pipeline data access for reading/writing a file based on a masked astropy QTable. """
    def __init__(self,
                 file: _Union[str, _Path],
                 file_format: str="ascii.fixed_width_two_line",
                 file_format_kwargs: _Dict[str, any] = None):
        """
        Initializes the QTableFileDal Dal class, associating it with a file in which to durably
        store its data.

        **NOTE**: This is not robust enough to be used where multiple clients are expected. When
        updates are persisted, the entire file is overwritten. There is no storage level locking
        mechanism so it will happily overwrite updates from elsewhere. If you need multiple clients
        and/or robust locking, the MariaDbTableDal is suitable.
...
class MariaDbTableDal(Dal):
    """ Pipeline Dal for storing data in a MariaDB database table. """
    ...
    def __init__(self, db_config: _Dict[str, any], table_name: str="working_set", ):
        ...
        print(f"Data will be stored in the `{table_name}` table of the",
              f"{self._db_config['host']}/{self._db_config['database']} database.")
    ...
    def create_working_set_table(cls,
        ...
        :returns: whether or not the table was created (True) or it already existed (False)
        """
    ...
    def _set_up_sql_where_clauses_for_execute(cls, prefix: str="", keyword: str="",
        ...
        Text is built up as '{prefix} [{keyword} {where-clauses}] {suffix}'. Example usage:
        ```python
        where = { "fitted_lcs": False, "locked_by": None }
        cursor.execute(*_set_up_sql_where_clauses_for_execute("SELECT * FROM tbl", "WHERE", where))
        ```
        """
        text = (prefix or "") + " "
        if len(where or {}) > 0:
...
def create_dal(typename: _Union[str, type[Dal]], verbose: bool=False, **kwargs):
    """
    ...
    if isinstance(typename, str):
        ...
        for subclass in get_subclasses(Dal):
            if subclass.__name__.casefold() in possible_names:
                dal_type = subclass
                break
    elif issubclass(typename, Dal):
        dal_type = typename

    if dal_type is None:
        raise KeyError(f"No Dal type like {typename} was found.")
    ...
    if verbose:
        print(f"Creating a {dal_type.__name__} with kwargs={_dict_to_str(expected_kwargs)}")
